...
public class StorageImpl implements Storage {
	/**
	 * Initial database index size - increasing it reduce number of index
	...
	final void freePage(int oid) {
		long pos = getPos(oid);
		Assert
				.that((pos & (dbFreeHandleFlag | dbPageObjectFlag)) == dbPageObjectFlag);
		if ((pos & dbModifiedFlag) != 0) {
	...
	public synchronized void open(String filePath, long pagePoolSize) {
		IFile file = filePath.startsWith("@") ? (IFile) new MultiFile(filePath
				.substring(1), readOnly, noFlush) : (IFile) new OSFile(
		...
		if (descListOid != 0) {
			...
			for (desc = descList; desc != null; desc = desc.next) {
				...
				if (desc.allocator != null) {
					...
					reserveLocation(allocator.getSegmentBase(), allocator
							.getSegmentSize());
				}
				checkIfFinal(desc);
			}
	...
	public synchronized <T> FieldIndex<T> createRandomAccessFieldIndex(
		...
		FieldIndex<T> index = caseInsensitive ? (FieldIndex<T>) new RndBtreeCaseInsensitiveFieldIndex<T>(
				type, fieldName, unique)
				: (FieldIndex<T>) new RndBtreeFieldIndex<T>(type, fieldName,
						unique);
		index.assignOid(this, 0, false);
		return index;
	}
	...
	public synchronized <T> FieldIndex<T> createRandomAccessFieldIndex(
		...
		FieldIndex<T> index = caseInsensitive ? (FieldIndex<T>) new RndBtreeCaseInsensitiveMultiFieldIndex(
				type, fieldNames, unique)
				: (FieldIndex<T>) new RndBtreeMultiFieldIndex(type, fieldNames,
						unique);
		index.assignOid(this, 0, false);
		return index;
	}
	...
	final Object unswizzle(ArrayPos obj, Class cls, Object parent,
		...
		if (oid < 0) {
			...
			case ClassDescriptor.tpClass:
				obj.offs = offs;
				return ClassDescriptor.loadClass(this, Bytes.unpackString(obj,
						encoding));
			case ClassDescriptor.tpLink: {
			...
			case ClassDescriptor.tpArrayOfRaw: {
				...
				for (int j = 0; j < len; j++) {
					Array.set(arr, j, unswizzle(obj, elemType, parent,
							recursiveLoading));
				}
				return arr;
			}
	...
	final int unpackObject(Object obj, ClassDescriptor desc,
		...
		for (int i = 0, n = all.length; i < n; i++) {
			...
			if (f == null || obj == null) {
				...
				case ClassDescriptor.tpClass: {
					ArrayPos pos = new ArrayPos(body, offs);
					Class cls = ClassDescriptor.loadClass(this, Bytes
							.unpackString(pos, encoding));
				...
				case ClassDescriptor.tpObject: {
					ArrayPos pos = new ArrayPos(body, offs);
					provider.set(f, obj, unswizzle(pos, f.getType(), parent,
							recursiveLoading));
					offs = pos.offs;
					continue;
				}
	...
	final int packValue(Object value, int offs, ByteBuffer buf)
		...
		if (value == null) {
			...
			if (c == Boolean.class) {
				...
				Bytes.pack2(buf.arr, offs + 4, (short) ((Character) value)
						.charValue());
				offs += 6;
			}
				...
				Bytes.pack4(buf.arr, offs + 4, Float
						.floatToIntBits(((Float) value).floatValue()));
				offs += 8;
			}
			else if (c == Double.class) {
				buf.extend(offs + 12);
				Bytes.pack4(buf.arr, offs, -2 - ClassDescriptor.tpDouble);
				Bytes.pack8(buf.arr, offs + 4, Double
	...
	final int swizzle(ByteBuffer buf, int offs, Object obj) throws Exception {
		if (obj instanceof IPersistent || obj == null) {
			...
			if (t == Boolean.class) {
				...
				Bytes.pack2(buf.arr, offs + 4, (short) ((Character) obj)
						.charValue());
				offs += 6;
			}
				...
				offs = buf.packString(offs, ClassDescriptor
						.getClassName((Class) obj));
			}
