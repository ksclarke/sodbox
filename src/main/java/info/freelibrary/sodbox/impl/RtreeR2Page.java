...
public class RtreeR2Page extends Persistent {
    ...
    RtreeR2Page(final Storage storage, final Object obj, final RectangleR2 r) {
    ...
    RtreeR2Page(final Storage storage, final RtreeR2Page root, final RtreeR2Page p) {
        branch = storage.createLink(card);
        branch.setSize(card);
        b = new RectangleR2[card];
        n = 2;
    ...
    final RtreeR2Page addBranch(final Storage storage, final RectangleR2 r, final Object obj) {
        if (n < card) {
            setBranch(n++, r, obj);
            return null;
        } else {
    ...
    final RectangleR2 cover() {
        final RectangleR2 r = new RectangleR2(b[0]);
        for (int i = 1; i < n; i++) {
            r.join(b[i]);
        }
        return r;
    ...
    void find(final RectangleR2 r, final ArrayList result, int level) {
    ...
    RtreeR2Page insert(final Storage storage, final RectangleR2 r, final Object obj, int level) {
        modify();
        if (--level != 0) {
            // not leaf page
            int i, mini = 0;
            ...
            for (i = 0; i < n; i++) {
                final double area = b[i].area();
                final double incr = RectangleR2.joinArea(b[i], r) - area;
                if (incr < minIncr) {
                    minIncr = incr;
                    minArea = area;
                    mini = i;
            ...
            final RtreeR2Page p = (RtreeR2Page) branch.get(mini);
            final RtreeR2Page q = p.insert(storage, r, obj, level);
            if (q == null) {
                // child was not split
                b[mini].join(r);
                return null;
    ...
    int remove(final RectangleR2 r, final Object obj, int level, final ArrayList reinsertList) {
        if (--level != 0) {
            for (int i = 0; i < n; i++) {
                if (r.intersects(b[i])) {
                    final RtreeR2Page pg = (RtreeR2Page) branch.get(i);
    ...
    final void setBranch(final int i, final RectangleR2 r, final Object obj) {
        b[i] = r;
        branch.setObject(i, obj);
    }

    final RtreeR2Page splitPage(final Storage storage, final RectangleR2 r, final Object obj) {
        ...
        while (groupCard0 + groupCard1 < card + 1 && groupCard0 < card + 1 - minFill && groupCard1 < card + 1 -
            ...
            for (i = 0; i < card; i++) {
                if (taken[i] == 0) {
                    final double diff = RectangleR2.joinArea(group0, b[i]) - groupArea0 - (RectangleR2.joinArea(
                            group1, b[i]) - groupArea1);
