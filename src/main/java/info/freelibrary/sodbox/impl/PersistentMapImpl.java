...
class PersistentMapImpl<K extends Comparable, V> extends PersistentResource
	...
	static class PersistentMapComparator<K extends Comparable, V> extends
		...
		public int compareMembers(PersistentMapEntry<K, V> m1,
				PersistentMapEntry<K, V> m2) {
	...
	public boolean containsKey(Object key) {
		if (index != null) {
			if (type == ClassDescriptor.tpValue) {
				...
				return ((Index) index).entryIterator(k, k, Index.ASCENT_ORDER)
						.hasNext();
			}
		}
		else {
	...
	public V get(Object key) {
		if (index != null) {
			if (type == ClassDescriptor.tpValue) {
				PersistentMapEntry<K, V> entry = ((SortedCollection<PersistentMapEntry<K, V>>) index)
						.get(key);
	...
	public Entry<K, V> getEntry(Object key) {
		if (index != null) {
			if (type == ClassDescriptor.tpValue) {
				return ((SortedCollection<PersistentMapEntry<K, V>>) index)
						.get(key);
			}
			else {
				V value = ((Index<V>) index).get(generateKey(key));
				return value != null ? new PersistentMapEntry((K) key, value)
						: null;
			}
		}
		else {
		...
		if (index == null) {
			...
			if (i < size && key.equals(((Comparable[]) keys)[i])) {
				...
				if (size == BTREE_TRESHOLD) {
					...
					if (type == ClassDescriptor.tpValue) {
						SortedCollection<PersistentMapEntry<K, V>> col = getStorage()
								.<PersistentMapEntry<K, V>> createSortedCollection(
										new PersistentMapComparator<K, V>(),
										true);
						index = col;

						for (i = 0; i < size; i++) {
							col.add(new PersistentMapEntry((K) keys[i], values
									.get(i)));
						}
						...
						Index<V> idx = getStorage().<V> createIndex(
								Btree.mapKeyType(type), true);
						index = idx;
					...
					if (size >= oldKeys.length) {
						Comparable[] newKeys = new Comparable[size + 1 > oldKeys.length * 2 ? size + 1
								: oldKeys.length * 2];
		...
		if (index != null) {
			if (type == ClassDescriptor.tpValue) {
				return new Iterator<Entry<K, V>>() {
					private Iterator<PersistentMapEntry<K, V>> i = ((SortedCollection<PersistentMapEntry<K, V>>) index)
							.iterator();
				...
				return new Iterator<Entry<K, V>>() {
					private Iterator<Entry<Object, V>> i = ((Index<V>) index)
							.entryIterator();
				...
				public boolean contains(Object k) {
					...
					if (e.getValue() != null) {
						return e.getValue().equals(
								PersistentMapImpl.this.get(e.getKey()));
					}
					else {
			...
			if (index != null) {
				if (type == ClassDescriptor.tpValue) {
					if (order == Index.ASCENT_ORDER) {
						...
						return new Iterator<Entry<K, V>>() {
							private ArrayList<PersistentMapEntry<K, V>> entries = ((SortedCollection<PersistentMapEntry<K, V>>) index)
									.getList(fromKey, toKey);
					...
					return new Iterator<Entry<K, V>>() {
						private Iterator<Entry<Object, V>> i = ((Index<V>) index)
								.entryIterator(fromKey, toKey, order);
				...
				entrySet = new AbstractSet<Entry<K, V>>() {
					...
					public boolean contains(Object k) {
						...
						if (e.getValue() != null) {
							...
							return PersistentMapImpl.this.containsKey(e
									.getKey())
									&& PersistentMapImpl.this.get(e.getKey()) == null;
						}
					}
	...
	public K lastKey() {
		if (index != null) {
			if (type == ClassDescriptor.tpValue) {
				ArrayList<PersistentMapEntry<K, V>> entries = ((SortedCollection<PersistentMapEntry<K, V>>) index)
						.getList(null, null);
				return entries.get(entries.size() - 1).key;
			}
			else {
				return (K) ((Index<V>) index)
						.entryIterator(null, null, Index.DESCENT_ORDER).next()
						.getKey();
			}
		}
		else {
