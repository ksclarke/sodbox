...
public class TtreePage extends Persistent {
    ...
    static class PageReference {
        ...
        PageReference(final TtreePage p) {
            pg = p;
        }
    }
    ...
    TtreePage(final Storage db, final Object mbr) {
        super(db);
        nItems = 1;
    ...
    final int balanceLeftBranch(final PageReference ref) {
        if (balance < 0) {
            balance = 0;
            return UNDERFLOW;
        } else if (balance == 0) {
            ...
            final TtreePage right = this.right;
            right.load();
            right.modify();
            if (right.balance >= 0) { // single RR turn
                ...
                final TtreePage left = right.left;
                left.load();
                left.modify();
    ...
    final int balanceRightBranch(final PageReference ref) {
        if (balance > 0) {
            balance = 0;
            return UNDERFLOW;
        } else if (balance == 0) {
            ...
            final TtreePage left = this.left;
            left.load();
            left.modify();
            if (left.balance <= 0) { // single LL turn
                ...
                final TtreePage right = left.right;
                right.load();
                right.modify();
    ...
    final boolean contains(final PersistentComparator comparator, final Object key) {
        int l, r, m, n;
        load();
        n = nItems;
    ...
    final boolean containsKey(final PersistentComparator comparator, final Object key) {
        int l, r, m, n;
        load();
        n = nItems;
        if (comparator.compareMemberWithKey(loadItem(0), key) < 0) {
            ...
            while (r < n) {
                final int diff = comparator.compareMemberWithKey(loadItem(r), key);
                if (diff == 0) {
                    return true;
                } else if (diff > 0) {
                    return false;
        ...
        for (l = 0; l < n; l++) {
            final int diff = comparator.compareMemberWithKey(loadItem(l), key);
            if (diff == 0) {
                return true;
            } else if (diff > 0) {
                return false;
    ...
    final boolean containsObject(final PersistentComparator comparator, final Object mbr) {
        int l, r, m, n;
        load();
        n = nItems;
    ...
    final boolean find(final PersistentComparator comparator, final Object minValue, final int minInclusive,
            final Object maxValue, final int maxInclusive, final ArrayList selection) {
        int l, r, m, n;
        load();
        n = nItems;
        if (minValue != null) {
    ...
    final int insert(final PersistentComparator comparator, final Object mbr, final boolean unique,
        ...
        if (diff <= 0) {
            ...
            if (left == null) {
                ...
                pg = ref.pg;
                ref.pg = left;
                final int result = left.insert(comparator, mbr, unique, ref);
        ...
        if (diff >= 0) {
            ...
            if (right == null) {
                ...
                pg = ref.pg;
                ref.pg = right;
                final int result = right.insert(comparator, mbr, unique, ref);
                if (result == NOT_UNIQUE) {
                    return NOT_UNIQUE;
                }
                modify();
    ...
    final Object loadItem(final int i) {
        final Object mbr = item[i];
        getStorage().load(mbr);
        return mbr;
    }
    ...
    final int remove(final PersistentComparator comparator, final Object mbr, final PageReference ref) {
        load();
        TtreePage pg;
        final int n = nItems;
        int diff = comparator.compareMembers(mbr, loadItem(0));
        if (diff <= 0) {
            if (left != null) {
                ...
                pg = ref.pg;
                ref.pg = left;
                final int h = left.remove(comparator, mbr, ref);
                left = ref.pg;
                ref.pg = pg;
                if (h == UNDERFLOW) {
        ...
        if (right != null) {
            ...
            pg = ref.pg;
            ref.pg = right;
            final int h = right.remove(comparator, mbr, ref);
            right = ref.pg;
            ref.pg = pg;
            if (h == UNDERFLOW) {
    ...
    final int toArray(final Object[] arr, int index) {
        load();
        if (left != null) {
