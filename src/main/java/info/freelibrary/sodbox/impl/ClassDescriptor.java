...
public final class ClassDescriptor extends Persistent {
	...
	static final String signature[] = { "boolean", "byte", "char", "short",
			"int", "long", "float", "double", "String", "Date", "Object",
			"Value", "Raw", "Link", "enum", "", "", "", "", "", "",
			"ArrayOfBoolean", "ArrayOfByte", "ArrayOfChar", "ArrayOfShort",
			"ArrayOfInt", "ArrayOfLong", "ArrayOfFloat", "ArrayOfDouble",
			"ArrayOfEnum", "ArrayOfString", "ArrayOfDate", "ArrayOfObject",
			"ArrayOfValue", "ArrayOfRaw", "ArrayOfLink", "ArrayOfEnum", "Class" };
	...
	static ReflectionProvider getReflectionProvider() {
		if (reflectionProvider == null) {
			try {
				...
				reflectionProvider = (ReflectionProvider) Class.forName(cls)
						.newInstance();
			}
			catch (Throwable x) {
	...
	public static boolean isEmbedded(Object obj) {
		if (obj != null) {
			...
			return obj instanceof IValue || obj instanceof Number
					|| cls.isArray() || cls == Character.class
	...
	static boolean treateAnyNonPersistentObjectAsValue = Boolean
			.getBoolean("sodbox.implicit.values");
	static boolean serializeNonPersistentObjects = Boolean
			.getBoolean("sodbox.serialize.transient.objects");

	ClassDescriptor() {
	}

	private static Class loadClass(String aName) throws ClassNotFoundException {
		ClassLoader loader = Thread.currentThread().getContextClassLoader();

		if (loader != null) {
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug(RB.getString("classDescriptor.loadClass.clFound"),
						new String[] { aName, loader.getClass().getName() });
			}
			...
			try {
				...
				LOGGER.debug(RB.getString("classDescriptor.loadClass.clError"),
						aName, details);
			}
		}
		else {
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug(RB
						.getString("classDescriptor.loadClass.clNotFound"));
			}
		}
	...
	private void locateConstructor() {
		try {
			...
			try {
				loadConstructor = cls
						.getDeclaredConstructor(sodboxConstructorProfile);
				constructorParams = new Object[] { this };
			}
			catch (NoSuchMethodException x2) {
				try {
					loadConstructor = getReflectionProvider()
							.getDefaultConstructor(cls);
					constructorParams = null;
				}
				catch (Exception x3) {
					throw new StorageError(StorageError.DESCRIPTOR_FAILURE,
							cls, x3);
				}
			}
			try {
	...
	ClassDescriptor(StorageImpl storage, Class cls) {
		...
		allFields = (FieldDescriptor[]) list.toArray(new FieldDescriptor[list
				.size()]);
		locateConstructor();
		resolved = true;
	}
	...
	public static Field locateField(Class scope, String name) {
		try {
			...
			throw new StorageError(StorageError.ACCESS_VIOLATION, scope
					.getName()
					+ "." + name, x);
		}
		return null;
	...
	public static String getClassName(Class cls) {
		...
		return (loader instanceof INamedClassLoader) ? ((INamedClassLoader) loader)
				.getName()
				+ ':' + cls.getName()
				: cls.getName();
	}

	public static Class loadClass(Storage aStorage, String aName) {
		if (aStorage != null) {
			int col = aName.indexOf(':');
			ClassLoader loader;

			if (col >= 0) {
				loader = aStorage.findClassLoader(aName.substring(0, col));
		...
		try {
			...
			throw new StorageError(StorageError.CLASS_NOT_FOUND, aName, x);
		}
	}
	...
	public void onLoad() {
		...
		for (int i = n; --i >= 0;) {
			FieldDescriptor fd = allFields[i];
			if (fd.field == null) {
				hierarchyLoop: for (scope = cls; scope != null; scope = scope
						.getSuperclass()) {
					try {
						Field f = scope.getDeclaredField(fd.fieldName);
						if ((f.getModifiers() & (Modifier.TRANSIENT | Modifier.STATIC)) == 0) {
