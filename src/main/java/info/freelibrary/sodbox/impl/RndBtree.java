...
class RndBtree<T> extends PersistentCollection<T> implements Index<T> {
    ...
    static abstract class BtreePage extends Persistent {
        ...
        private void countChildren(final int i, final int height) {
        ...
        int handlePageUnderflow(final int r, final BtreeKey rem, final int height) {
            ...
            if (r < nItems) { // exists greater page
                ...
                if (an + bn > items.size()) {
                    ...
                    memcpy(a, an, b, 0, bn);
                    b.deallocate();
                    final int nMergedChildren = nChildren[r + 1];
        ...
        int indexOf(final Key key, int height) {
            int l = 0;
            final int n = nItems;
            int r = n;
            height -= 1;
        ...
        private void insert(final BtreeKey key, final int i, final int height) {
            insert(key, i);
            if (height != 0) {
    ...
    class BtreeSelectionEntryIterator extends BtreeSelectionIterator<Map.Entry<Object, T>> {
        ...
        BtreeSelectionEntryIterator(final int order) {
            super(order);
        }
    ...
    class BtreeSelectionIterator<E> extends IterableIterator<E> implements PersistentIterator {
        ...
        BtreeSelectionIterator(final int order) {
            this.order = order;
        }
    ...
    RndBtree(final Class cls, final boolean unique) {
        this.unique = unique;
        type = checkType(cls);
    }

    RndBtree(final int type, final boolean unique) {
        this.type = type;
        this.unique = unique;
    }

    final void allocateRootPage(final BtreeKey ins, final int height) {
