...
public class ReplicationMasterFile implements IFile, Runnable {
     ...
     * @param pageTimestampFilePath path to the file with pages timestamps. This file is used for synchronizing with
     *        master content of newly attached node
     */
    public ReplicationMasterFile(final IFile file, final String[] hosts, final boolean ack) {
    ...
    private ReplicationMasterFile(final ReplicationMasterStorageImpl storage, final IFile file, final int port,
            final String[] hosts, final boolean ack, final String pageTimestampFilePath) {
        ...
        if (pageTimestampFilePath != null) {
            pageTimestampFile = new OSFile(pageTimestampFilePath, false, storage != null && storage.myNoFlush);
        ...
        if (port >= 0) {
            storage.setProperty("sodbox.alternative.btree", Boolean.TRUE); // prevent direct modification of pages
            try {
     ...
     *        master content of newly attached node
     */
    public ReplicationMasterFile(final ReplicationMasterStorageImpl storage, final IFile file,
            final String pageTimestampFile) {
        this(storage, file, storage.port, storage.hosts, storage.myReplicationAck, pageTimestampFile);
    }

    private void addConnection(final Socket s) {
        ...
        synchronized (mutex) {
            ...
            final Thread t = new SynchronizeThread(n);
    ...
    public void close() {
        ...
        for (final Thread t : syncThreads) {
            if (t != null) {
                try {
                    t.join();
                } catch (final InterruptedException x) {
    ...
    protected void connect(final int i) {
        String host = hosts[i];
        ...
        Socket socket = null;
        try {
            final int maxAttempts = storage != null ? storage.mySlaveConnectionTimeout : MAX_CONNECT_ATTEMPTS;
     ...
     */
    public boolean handleError(final String host) {
        System.err.println("Failed to establish connection with host " + host);
        return storage != null && storage.myListener != null ? storage.myListener.replicationError(host) : false;
    }
                    ...
                    if (dirtyPageTimestampMap[i] != 0) {
                        for (int j = 0; j < 32; j++) {
                            if ((dirtyPageTimestampMap[i] & 1 << j) != 0) {
                                ...
                                final long pos = pageNo << Page.pageSizeLog;
    ...
    private void synchronizeNode(final int i) {
        final long size = storage.getDatabaseSize();
        Socket s;
        OutputStream os = null;
        InputStream is = null;
        synchronized (mutex) {
            ...
            do {
                ...
                for (long pos = 0; pos < size; pos += Page.pageSize) {
                    ...
                    synchronized (storage) {
                        synchronized (storage.myObjectCache) {
                            final Page pg = storage.myPagePool.getPage(pos);
                            Bytes.pack8(txBuf, 0, pos);
                            System.arraycopy(pg.data, 0, txBuf, 8, Page.pageSize);
                            storage.myPagePool.unfix(pg);
                            if (syncNodeTimestamps != null) {
                                Bytes.pack4(txBuf, Page.pageSize + 8, pageNo < pageTimestamps.length
                                        ? pageTimestamps[pageNo] : 0);
                            }
                        }
                    }
                    storage.myPagePool.getPage(pos);
                    synchronized (s) {
                        os.write(txBuf);
