...
class BtreePage {
    ...
    static int allocate(final StorageImpl db, final int root, final int type, final BtreeKey ins) {
        final int pageId = db.allocatePage();
        final Page pg = db.putPage(pageId);
        setnItems(pg, 1);
        if (type == ClassDescriptor.tpString) {
            final char[] sval = (char[]) ins.key.oval;
            ...
            final byte[] bval = (byte[]) ins.key.oval;
            final int len = bval.length;
            setSize(pg, len);
    ...
    static int compactifyByteArrays(final Page pg, int m) {
        int i, j, offs, len, n = getnItems(pg);
        final int[] size = new int[keySpace + 1];
        final int[] index = new int[keySpace + 1];
        if (m == 0) {
            return n;
        }
    ...
    static int compactifyStrings(final Page pg, int m) {
        int i, j, offs, len, n = getnItems(pg);
        final int[] size = new int[keySpace / 2 + 1];
        final int[] index = new int[keySpace / 2 + 1];
        if (m == 0) {
            return n;
        }
    ...
    final static int compare(final Key key, final Page pg, final int i) {
        long i8;
        int i4;
        float r4;
        double r8;
    ...
    final static int comparePrefix(final char[] key, final Page pg, final int i) {
        final int alen = key.length;
        final int blen = BtreePage.getKeyStrSize(pg, i);
        final int minlen = alen < blen ? alen : blen;
        int offs = BtreePage.getKeyStrOffs(pg, i) + BtreePage.firstKeyOffs;
        final byte[] b = pg.data;
        for (int j = 0; j < minlen; j++) {
            final int diff = key[j] - (char) Bytes.unpack2(b, offs);
            if (diff != 0) {
                return diff;
            }
            offs += 2;
    ...
    final static int compareStr(final Key key, final Page pg, final int i) {
        final char[] chars = (char[]) key.oval;
    ...
    static void exportPage(final StorageImpl db, final XMLExporter exporter, final int pageId, final int type,
        ...
        try {
            ...
            if (--height != 0) {
                ...
                if (type == ClassDescriptor.tpString || type == ClassDescriptor.tpArrayOfByte) { // page of strings
                    for (i = 0; i < n; i++) {
                        exporter.exportAssoc(getKeyStrOid(pg, i), pg.data, BtreePage.firstKeyOffs + BtreePage
                                .getKeyStrOffs(pg, i), BtreePage.getKeyStrSize(pg, i), type);
                    }
                } else {
                    for (i = 0; i < n; i++) {
                        exporter.exportAssoc(getReference(pg, maxItems - 1 - i), pg.data, BtreePage.firstKeyOffs + i *
                                ClassDescriptor.sizeof[type], ClassDescriptor.sizeof[type], type);
    ...
    static boolean find(final StorageImpl db, final int pageId, final Key firstKey, final Key lastKey,
            final Btree tree, int height, final ArrayList result) {
        final Page pg = db.getPage(pageId);
        int l = 0;
        final int n = getnItems(pg);
        int r = n;
    ...
    static int getKeyStrOffs(final Page pg, final int index) {
        return Bytes.unpack2(pg.data, firstKeyOffs + index * 8 + 6);
    }

    static int getKeyStrOid(final Page pg, final int index) {
        return Bytes.unpack4(pg.data, firstKeyOffs + index * 8);
    }

    static int getKeyStrSize(final Page pg, final int index) {
        return Bytes.unpack2(pg.data, firstKeyOffs + index * 8 + 4);
    }

    static int getnItems(final Page pg) {
        return Bytes.unpack2(pg.data, 0);
    }

    static int getReference(final Page pg, final int index) {
        return Bytes.unpack4(pg.data, firstKeyOffs + index * 4);
    }

    static int getSize(final Page pg) {
        return Bytes.unpack2(pg.data, 2);
    }

    static int handlePageUnderflow(final StorageImpl db, final Page pg, final int r, final int type,
            final BtreeKey rem, final int height) {
        final int nItems = getnItems(pg);
        if (type == ClassDescriptor.tpString) {
            final Page a = db.putPage(getKeyStrOid(pg, r));
            int an = getnItems(a);
            if (r < nItems) { // exists greater page
                ...
                if (merged_size > keySpace) {
                    ...
                    while (true) {
                        i += 1;
                        final int delta = (an + i) * strKeySize + size_a + addSize * 2 - ((bn - i) * strKeySize +
                                size_b - subSize * 2);
                        if (delta >= 0) {
                            if (delta >= -prevDelta) {
                                i -= 1;
                    ...
                    if (i > 0) {
                        k = i;
                        if (height != 1) {
                            final int len = getKeyStrSize(pg, r);
                    ...
                    db.myPagePool.unfix(a);
                    db.myPagePool.unfix(b);
                    return result;
                } else { // merge page b to a
                    if (height != 1) {
                        final int r_len = getKeyStrSize(pg, r);
                ...
                if (merged_size > keySpace) {
                    ...
                    if (height != 1) {
                        ...
                        final int len = getKeyStrSize(pg, r - 1);
            ...
            if (r < nItems) { // exists greater page
                ...
                if (merged_size > keySpace) {
                    ...
                    while (true) {
                        i += 1;
                        final int delta = (an + i) * strKeySize + size_a + addSize - ((bn - i) * strKeySize + size_b -
                                subSize);
                        if (delta >= 0) {
                            if (delta >= -prevDelta) {
                                i -= 1;
            ...
            final Page a = db.putPage(getReference(pg, maxItems - r - 1));
            int an = getnItems(a);
            final int itemSize = ClassDescriptor.sizeof[type];
            if (r < nItems) { // exists greater page
    ...
    static int insert(final StorageImpl db, int pageId, final Btree tree, final BtreeKey ins, int height,
            final boolean unique, final boolean overwrite) {
        ...
        try {
            if (tree.type == ClassDescriptor.tpString) {
                ...
                pg = null;
                pg = db.putPage(pageId);
                final int itemSize = ClassDescriptor.sizeof[tree.type];
                final int max = keySpace / (4 + itemSize);
                if (n < max) {
                    ...
                    final Page b = db.putPage(pageId);
                    Assert.that(n == max);
                    final int m = (max + 1) / 2;
                    if (r < m) {
    ...
    static int insertByteArrayKey(final StorageImpl db, final Page pg, final int r, final BtreeKey ins,
            final int height) {
        int nItems = getnItems(pg);
        int size = getSize(pg);
        final int n = height != 0 ? nItems + 1 : nItems;
        final byte[] bval = (byte[]) ins.key.oval;
        // insert before e[r]
        final int len = bval.length;
        if (size + len + (n + 1) * strKeySize <= keySpace) {
            ...
            for (int bn = 0, i = 0;; bn += 1) {
                ...
                final int delta = moved + addSize + (bn + 1) * strKeySize - (j * strKeySize + size - subSize +
                        inserted);
                if (delta >= -prevDelta) {
                    if (height == 0) {
    ...
    static int insertStrKey(final StorageImpl db, final Page pg, final int r, final BtreeKey ins, final int height) {
        int nItems = getnItems(pg);
        int size = getSize(pg);
        final int n = height != 0 ? nItems + 1 : nItems;
        // insert before e[r]
        ...
        if (size + len * 2 + (n + 1) * strKeySize <= keySpace) {
            ...
            for (int bn = 0, i = 0;; bn += 1) {
                ...
                final int delta = moved + addSize * 2 + (bn + 1) * strKeySize - (j * strKeySize + size - subSize * 2 +
                        inserted);
                if (delta >= -prevDelta) {
                    if (height == 0) {
                        ins.getStr(b, bn - 1);
    ...
    static int markPage(final StorageImpl db, final int pageId, final int type, int height) {
        int nPages = 1;
        final Page pg = db.getGCPage(pageId);
        try {
            int i;
            final int n = getnItems(pg);
            if (--height != 0) {
    ...
    static void memcpy(final Page dst_pg, final int dst_idx, final Page src_pg, final int src_idx, final int len,
            final int itemSize) {
        System.arraycopy(src_pg.data, firstKeyOffs + src_idx * itemSize, dst_pg.data, firstKeyOffs + dst_idx *
                itemSize, len * itemSize);
    }

    static boolean prefixSearch(final StorageImpl db, final int pageId, final char[] key, int height,
    ...
    static void purge(final StorageImpl db, final int pageId, final int type, int height) {
        if (--height != 0) {
            final Page pg = db.getPage(pageId);
    ...
    static int remove(final StorageImpl db, final int pageId, final Btree tree, final BtreeKey rem, int height) {
        Page pg = db.getPage(pageId);
        try {
    ...
    static int removeByteArrayKey(final Page pg, final int r) {
        final int len = getKeyStrSize(pg, r);
        final int offs = getKeyStrOffs(pg, r);
        int size = getSize(pg);
        final int nItems = getnItems(pg);
    ...
    static int removeStrKey(final Page pg, final int r) {
        final int len = getKeyStrSize(pg, r) * 2;
        final int offs = getKeyStrOffs(pg, r);
        int size = getSize(pg);
        final int nItems = getnItems(pg);
        if ((nItems + 1) * strKeySize >= keySpace) {
    ...
    static int replaceByteArrayKey(final StorageImpl db, final Page pg, final int r, final BtreeKey ins,
            final int height) {
    ...
    static int replaceStrKey(final StorageImpl db, final Page pg, final int r, final BtreeKey ins, final int height) {
        ins.oid = getKeyStrOid(pg, r);
        removeStrKey(pg, r);
        return insertStrKey(db, pg, r, ins, height);
    }

    static void setKeyBytes(final Page pg, final int offs, final byte[] bytes) {
        System.arraycopy(bytes, 0, pg.data, firstKeyOffs + offs, bytes.length);
    }

    static void setKeyStrChars(final Page pg, int offs, final char[] str) {
    ...
    static void setKeyStrOffs(final Page pg, final int index, final int offs) {
        Bytes.pack2(pg.data, firstKeyOffs + index * 8 + 6, (short) offs);
    }

    static void setKeyStrOid(final Page pg, final int index, final int oid) {
        Bytes.pack4(pg.data, firstKeyOffs + index * 8, oid);
    }

    static void setKeyStrSize(final Page pg, final int index, final int size) {
        Bytes.pack2(pg.data, firstKeyOffs + index * 8 + 4, (short) size);
    }

    static void setnItems(final Page pg, final int nItems) {
        Bytes.pack2(pg.data, 0, (short) nItems);
    }

    static void setReference(final Page pg, final int index, final int oid) {
        Bytes.pack4(pg.data, firstKeyOffs + index * 4, oid);
    }

    static void setSize(final Page pg, final int size) {
        Bytes.pack2(pg.data, 2, (short) size);
    }

    static int traverseForward(final StorageImpl db, final int pageId, final int type, int height,
            final Object[] result, int pos) {
        final Page pg = db.getPage(pageId);
        int oid;
        try {
            int i;
