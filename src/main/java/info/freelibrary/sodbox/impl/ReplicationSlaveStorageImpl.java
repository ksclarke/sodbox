...
public abstract class ReplicationSlaveStorageImpl extends StorageImpl implements
		ReplicationSlaveStorage, Runnable {
	...
	/**
	 * When overridden by base class this method performs socket error handling
	 * 
	...
	public void run() {
		...
		while (listening) {
			...
			if (pos == 0) {
				if (!initialized
						&& buf[PAGE_DATA_OFFSET + DB_HDR_INITIALIZED_OFFSET] != 0) {
					synchronized (init) {
						initialized = true;
						init.notify();
					}
