...
public class FullTextSearchResult {
    ...
    public FullTextSearchResult(final FullTextSearchHit[] hits, final int estimation) {
        this.hits = hits;
     ...
     */
    public FullTextSearchResult merge(final FullTextSearchResult another) {
        if (hits.length == 0 || another.hits.length == 0) {
            return new FullTextSearchResult(new FullTextSearchHit[0], 0);
        }
        final FullTextSearchHit[] joinHits = new FullTextSearchHit[hits.length + another.hits.length];
        ...
        for (int i = 1; i < joinHits.length; i++) {
            if (joinHits[i].oid == joinHits[i - 1].oid) {
                joinHits[n++] = new FullTextSearchHit(joinHits[i].storage, joinHits[i].oid, joinHits[i - 1].rank +
                        joinHits[i].rank);
                i += 1;
            }
        }
        ...
        return new FullTextSearchResult(joinHits, Math.min(estimation * n / hits.length, another.estimation * n /
                another.hits.length));
    }
}