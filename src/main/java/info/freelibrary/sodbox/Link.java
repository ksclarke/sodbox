 ...
 * embedded (when references to the related objects are stored in relation owner
 * object itself) and standalone (when relation is separate object, which
 * contains the reference to the relation owner and relation members). Both
 * kinds of relations implements Link interface. Embedded relation is created by
 * Storage.createLink method and standalone relation is represented by Relation
...
public interface Link<T> extends List<T>, RandomAccess {
	...
	/**
	 * Return array with relation members. Members are not loaded and size of
	 * the array can be greater than actual number of members.
	 * 
	 * @return array of object with relation members used in implementation of
	 *         Link class
	 */
	public Object[] toRawArray();

	/**
	 * Get all relation members as array. The runtime type of the returned array
	...
	/**
	 * Replace all direct references to linked objects with stubs. This method
	 * is needed two avoid memory exhaustion in case when there is a large
	 * number of objects in database, mutually referencing each other (each
	 * object can directly or indirectly be accessed from other objects).
	 */
	public void unpin();
	 ...
	 * equivalent to the following peace of code: { Iterator i =
	 * index.iterator(); while (i.hasNext())
	 * ((IPersistent)i.next()).deallocate(); index.clear(); } Please notice that
	 * this method doesn't check if there are some other references to the
	 * deallocated objects. If deallocated object is included in some other
	 * index or is referenced from some other objects, then after deallocation
	 * there will be dangling references and dereferencing them can cause
