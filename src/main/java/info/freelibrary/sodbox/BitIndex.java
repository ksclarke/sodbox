 ...
 * has associated mask of 32 bites. Meaning of bits is application dependent. Usually each bit stands for some binary
 * or boolean property, for example "sex", but it is possible to use group of bits to represent enumerations with more
 * possible values.
 */
public interface BitIndex<T> extends IPersistent, IResource {
     ...
     * @throws StorageError KEY_NOT_FOUND exception if there is no object in the index
     */
    ...
    /**
     * Get iterator for selecting objects with specified properties. To select all record this method should be
     * invoked with (0, 0) parameters This iterator doesn't support remove() method. It is not possible to modify bit
