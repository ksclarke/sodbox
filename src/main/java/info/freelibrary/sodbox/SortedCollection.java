...
/**
 * Interface of sorted collection. Sorted collections keeps in members in order
 * specified by comparator. Members in the collections can be located using key
 * or range of keys. The SortedCollection is efficient container of objects for
 * in-memory databases. For databases which size is significantly larger than
 * size of page pool, operation with SortedList can cause trashing and so very
 * bad performance. Unlike other index structures SortedCollection doesn't store
 * values of keys and so search in the collection requires fetching of its
 * members.
 */
public interface SortedCollection<T> extends IPersistent, IResource,
		Collection<T> {
	...
	/**
	 * Get members which key value belongs to the specified range. Either from
	 ...
	 *         ordered by key value
	 */
	public Object[] get(Object from, boolean fromInclusive, Object till,
	 ...
	 *         ordered by key value
	 */
	public ArrayList<T> getList(Object from, boolean fromInclusive,
	 ...
	 *         <code>false</code> if collection was declared as unique and there
	 *         is already member with such value of the key in the collection.
	 */
	...
	/**
	 * Get iterator for traversing all collection members. This iterator fetches
	 * all selected objects in memory. It supports remove() method and all
	 * modifications of collection during iteration has no influence on
	 * iterator.
	 * 
	...
	/**
	 * Get iterator for traversing collection members with key belonging to the
	 * specified range. This iterator fetches all selected objects in memory. It
	 * supports remove() method and all modifications of collection during
