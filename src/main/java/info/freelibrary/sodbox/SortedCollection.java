...
public interface SortedCollection<T> extends IPersistent, IResource, Collection<T> {
     ...
     * @exception StorageError(StorageError.KEY_NOT_UNIQUE) exception if there are more than one objects in the
     *            collection with specified value of the key.
     */
     ...
     *
     * @param from inclusive low boundary. If <code>null</code> then low boundary is not specified.
     * @param till inclusive high boundary. If <code>null</code> then high boundary is not specified. High boundary
