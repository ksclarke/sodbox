 ...
 * standard OSFile can significantly increase speed of application in some
 * cases.
 */
public class MappedFile implements IFile {
	...
	private final void checkSize(long size) throws IOException {
		if (size > mapSize) {
			long newSize = mapSize < Integer.MAX_VALUE / 2 ? mapSize * 2
					: Integer.MAX_VALUE;
	...
	public MappedFile(String filePath, long initialSize, boolean readOnly) {
		try {
			...
			map = chan.map(readOnly ? FileChannel.MapMode.READ_ONLY
