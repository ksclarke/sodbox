...
/**
 * Interface of indexed field. Index is used to provide fast access to the
 * object by the value of indexed field. Objects in the index are stored ordered
 * by the value of indexed field. It is possible to select object using exact
 * value of the key or select set of objects which key belongs to the specified
 * interval (each boundary can be specified or unspecified and can be inclusive
 * or exclusive) Key should be of scalar, String, java.util.Date or persistent
 * object type.
 */
public interface FieldIndex<T> extends GenericIndex<T> {
	...
	/**
	 * Associate new object with the key specified by object field value. If
	 * there is already object with such key in the index, then it will be
