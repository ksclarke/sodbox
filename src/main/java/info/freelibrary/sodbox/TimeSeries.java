...
/**
 * <p>
 * Time series interface. Time series class is used for efficient handling of
 * time series data. Usually time series contains a very large number if
 * relatively small elements which are usually accessed in successive order. To
 * avoid overhead of loading from the disk each particular time series element,
 * this class group several subsequent time series elements together and store
 * them as single object (block).
 * </p>
 * <p>
 * As far as Java currently has no templates and Sodbox need to know format of
 * block class, it is the responsibility of programmer to create block
 * implementation derived from TimeSeries.Block class and containing array of
 * time series elements. Size of this array specifies the size of the block.
 * </p>
 */
public interface TimeSeries<T extends TimeSeries.Tick> extends IPersistent,
		IResource {

	/**
	 * Interface for TimeSeries element. You should derive your time series
	 * element from this class and implement getTime method.
	 */
	...
	/**
	 * Abstract base class for time series block. Programmer has to define its
	 * own block class derived from this class containing array of time series
	 * elements and providing getTicks() method to access this array. It is
	 * better no to initialize this array in constructor (because it will be
	 * also used when block will be loaded from the disk), but check in
	 * getTicks() method that array is null, and if so - create new array.
	 */
	public static abstract class Block extends Persistent {
		 ...
		 * 
		 * @return preallocated array of time series element. Only
		 *         <code>used</code> items of this array actually contains time
		 *         series elements. But all array items should be not null and
	...
	/**
	 * Get forward iterator through all time series elements This iterator
	 * doesn't support remove() method. It is not possible to modify time series
	 * during iteration.
	 * 
	...
	/**
	 * Get forward iterator for time series elements belonging to the specified
	 * range This iterator doesn't support remove() method. It is not possible
	...
	/**
	 * Get iterator through all time series elements This iterator doesn't
	 * support remove() method. It is not possible to modify time series during
	 * iteration.
	 * 
