 ...
 * by key. Object in the index are stored ordered by key value. It is possible
 * to select object using exact value of the key or select set of objects which
 * key belongs to the specified interval (each boundary can be specified or
 * unspecified and can be inclusive or exclusive) Key should be of scalar,
 * String, java.util.Date or persistent object type.
 */
public interface Index<T> extends GenericIndex<T> {
	 ...
	 * @param obj object associated with this key. Object can be not yet
	...
	/**
	 * Associate new value with the key. If there is already object with such
	 * key in the index, then it will be removed from the index and new value
	...
	/**
	 * Associate new value with specified key. If there is already object with
	 * such key in the index, then it will be removed from the index and new
	 ...
	 * equivalent to th following peace of code: { Iterator i =
