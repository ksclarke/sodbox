...
public class CompressedReadWriteFile implements IFile {
	...
	long allocate(int bitSize) {
		...
		if (pos < 0) {
			pos = Bitmap.allocate(bitmap, bitmapStart, Bitmap.locateHoleEnd(
					bitmap, bitmapPos), bitSize);
			if (pos < 0) {
				byte[] newBitmap = new byte[bitmap.length
						+ bitmapExtensionQuantum];
				System.arraycopy(bitmap, 0, newBitmap, 0, bitmap.length);
				pos = Bitmap.allocate(newBitmap, Bitmap.locateBitmapEnd(
						newBitmap, bitmap.length), newBitmap.length, bitSize);
				Assert.that(pos >= 0);
				bitmap = newBitmap;
			}
		}
