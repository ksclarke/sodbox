...
public class StorageFactory {
     ...
     * @param aReplicationMasterNode name of the host where replication master is running
     * @param aMasterPort replication master socket port to which connection should be established
     * @param aPageTimestampFile path to the file with pages timestamps. This file is used for synchronizing with master
     *        content of newly attached node
     * @return new instance of the slave storage (unopened, you should explicitly invoke open method)
     */
    public ReplicationSlaveStorage addReplicationSlaveStorage(final String aReplicationMasterNode,
     ...
     * @param aAsyncBufSize if value of this parameter is greater than zero then replication will be asynchronous, done
     *        by separate thread and not blocking main application. Otherwise data is send to the slave nodes by the
     *        same thread which updates the database. If space asynchronous buffer is exhausted, then main thread will
    ...
    /**
     * Create new instance of the static slave node of replicated storage. The address of this host should be specified
     * in the replicationSlaveNodes parameter of createReplicationMasterStorage method. When replication master is
     * started it tries to establish connection with all of the specified nodes.
     *
